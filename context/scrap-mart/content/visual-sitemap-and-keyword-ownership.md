# Scrap Mart Visual Sitemap And Keyword Ownership

Last updated: 26 June 2026.

This is the cleaner website structure to use for the client approval stage.

## Simple Strategy

- Homepage: brand, category, trust, two-branch positioning.
- Location pages: own Auckland and Christchurch city terms at launch. Suburb terms are supporting proof inside those pages, not standalone pages yet.
- Price pages: own price-related search terms.
- What We Buy pages: own metal/category/grade search terms.
- Service pages: own collection, pickup, commercial, industrial, factory, construction/demolition, recurring-contract, and household drop-off search terms.
- Resources: answer common questions and support SEO/AI-search visibility.
- Bin services: removed from the current structure as requested by the client on 24 June 2026.

## Visual Tree

```text
/
├── Scrap Metal Prices
│   ├── Auckland Prices
│   └── Christchurch Prices
├── What We Buy
│   ├── Copper
│   ├── Copper Cable
│   ├── Aluminium
│   ├── Brass
│   ├── Stainless Steel
│   ├── Steel And Iron
│   ├── Lead
│   ├── Batteries
│   ├── Electric Motors
│   ├── Radiators
│   ├── Air Conditioners And Heat Pumps
│   ├── Hot Water Cylinders
│   ├── Zinc
│   └── E-Waste
├── Services
│   ├── Scrap Metal Collection
│   ├── Commercial Scrap Metal Recycling
│   ├── Industrial Scrap Metal Recycling
│   ├── Factory Scrap Removal
│   ├── Construction And Demolition Scrap Removal
│   ├── Scrap Metal Pick-Up
│   ├── Regular Scrap Collection Contracts
│   └── Household Scrap Drop-Off
├── Locations
│   ├── Auckland
│   │   └── Mentions Onehunga, Mt Roskill, Penrose, Mount Eden, Greenlane, Mangere, Manukau, East Tamaki, Flatbush, South Auckland, New Lynn, Henderson and West Auckland in-page
│   └── Christchurch
│       └── Mentions Hillsborough branch address and Christchurch service areas in-page
├── Resources
│   ├── How Scrap Metal Prices Are Calculated
│   ├── Copper Grades Explained
│   ├── Public Scrap Metal Pricing List
│   ├── Lead Acid Battery Recycling
│   ├── Hot Water Cylinder Scrap
│   ├── Aluminium Scrap In Christchurch
│   ├── Scrap Cable Prices
│   ├── Whiteware Recycling
│   ├── What Metals Are Worth The Most?
│   └── Items We Do Not Accept
├── About
├── Reviews
├── Gallery
├── FAQs
└── Contact
```

## Keyword Ownership Rules

| Area | Owns | Does not own |
| --- | --- | --- |
| Homepage | Scrap Mart, scrap metal buyers, scrap metal recycling, two-branch trust | Exact Auckland or Christchurch ranking terms |
| Auckland location page | scrap metal buyers Auckland, scrap metal recycling Auckland, scrap yard Auckland | Onehunga-only suburb terms as standalone page targets |
| Christchurch location page | scrap metal buyers Christchurch, scrap metal recycling Christchurch, scrap yard Christchurch | Auckland or NZ-wide terms |
| What We Buy hub | what scrap metals do you buy, metals we buy, scrap metal accepted | City buyer terms |
| Metal pages | copper scrap buyers, aluminium scrap buyers, scrap battery buyers, zinc scrap buyers, etc. | City terms unless used naturally |
| Service pages | commercial scrap metal recycling, scrap metal collection, scrap metal pick-up, household scrap drop-off | Metal-grade terms |
| Price pages | scrap metal prices Auckland, scrap metal prices Christchurch | General service terms |
